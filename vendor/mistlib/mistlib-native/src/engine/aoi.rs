...
use super::{rust_node_id, SessionCtx, ENGINE};
...
impl SessionCtx {
    ...
    pub async fn check_and_dispatch_aoi(&self) {
        ...
        let (aoi_entered, aoi_left) = {
            ...
            let self_id = ENGINE.self_id.lock().unwrap().clone();
        ...
        for id in aoi_entered {
            dispatch_event(EVENT_AOI_ENTERED, &self.room_id, &id, b"entered");
        }
        for id in aoi_left {
            dispatch_event(EVENT_AOI_LEFT, &self.room_id, &id, b"left");
        }

        {
            ...
            dispatch_event(EVENT_AOI_NODES, &self.room_id, &rust_node_id(), &json);
        }

        if let Some(l1) = self.l1_notifier.as_ref() {
            let store = self.node_store.lock().unwrap();
            let aoi_lock = self.aoi_nodes.lock().unwrap();
            for id in &*aoi_lock {
                if let Some(n) = store.nodes.get(id) {
                    l1.notify_node_position_updated(id, n.position.x, n.position.y, n.position.z);
                }
            }
        }
    }
    ...
    pub async fn check_and_dispatch_neighbors(&self) {
        let connected_ids = if let Some(overlay) = &self.overlay {
        ...
        if self.should_dispatch_all_connections_lost(connected_ids.len()) {
            let from = ENGINE.self_id.lock().unwrap().clone();
            ...
            .to_string()
            .into_bytes();
            dispatch_event(EVENT_ALL_CONNECTIONS_LOST, &self.room_id, &from, &payload);
        }
        ...
        if !nodes.is_empty() {
            dispatch_event(EVENT_NEIGHBORS, &self.room_id, &rust_node_id(), &nodes);
        }
    }
}
