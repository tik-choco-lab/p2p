...
use super::invite::{
    derive_discovery_proof, derive_invite_scope, derive_invite_secret, derive_room_scope,
    derive_topology_rank,
};
...
pub use validation::is_room_mailbox_message;
use validation::{
...
const MAX_EVENT_CLOCK_SKEW_SECONDS: u64 = 300;
...
pub fn message_filter(config: &NostrCodecConfig, room_id: &str) -> NostrFilter {
    NostrFilter {
...
pub fn decode_message_event<C: NostrCrypto>(
    ...
    match event.tag_value(TAG_P) {
        Some(pubkey) if pubkey == identity.public_key => {
            let expected_scope = crypto.message_scope(identity, &event.pubkey, room_id)?;
            if !event.has_tag_value(TAG_INVITE_SCOPE, &expected_scope) {
                return Err(MistError::Signaling(
                    "Nostr message pair scope mismatch".to_string(),
                ));
            }
        }
        Some(_) => {
